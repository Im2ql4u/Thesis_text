...
$\{\phi_1,\ldots,\phi_{N/2}\}$, fixing the spin labels makes the spin-orbital determinant
block diagonal, with one spatial determinant for each spin:
\[
\Psi_{\mathrm{closed}}(X)
=\frac{1}{(N/2)!}\;
\det[\phi_a(\mathbf r_i)]_{i\in\uparrow,a=1..N/2}\;
\det[\phi_a(\mathbf r_j)]_{j\in\downarrow,a=1..N/2},
\]
which is the computational ansatz used here. This fixes $S_z=0$, but it does not explicitly
project the interacting trial state onto a total-spin eigenstate. The expression has the
\emph{form} of a restricted closed-shell determinant, but
...
\end{equation}
therefore vanishes for an exact eigenstate. It is often a useful measure of wavefunction
quality, but need not decrease monotonically with the variational energy. It is independent of any external
...
DMC reduces the amplitude error of a trial state by imaginary-time projection. In imaginary time
...
$w=\exp[-(E_L(\mathbf R)-E_T)\Delta\tau]$. In the standard importance-sampled formulation,
the stationary walker density is the mixed distribution proportional to
$\Psi_T\Phi_0$, where $\Phi_0$ is the projected state; it is not generally
$|\Phi_0|^2$.
...
proxy for the population risk $\mathcal R(\theta)=\mathbb E_P[\ell(f_\theta(X),Y)]$. Much of
the standard machinery concerns the gap between the two: overfitting, the bias--variance
tradeoff, regularisation, held-out validation~\cite{HastieTibshiraniFriedman2009-ESL,Berner_Grohs_Kutyniok_Petersen_2022}.
...
unaffordable, so it is estimated from a mini-batch. For independent, uniformly drawn batches
this estimate is unbiased, and its variance sets the noise scale of the
trajectory~\cite{wu2022alignmentpropertysgdnoise}. Momentum
...
\end{equation}
the Fisher--Rao metric of the state written in terms of the amplitude score. Every state
...
is. How far a damped natural-gradient step departs from a Euclidean step also depends on the
gradient direction, regularisation and spectral cutoff. Chapter~\ref{ch:kernel} shows that
$\kappa$ alone does not decide when SR is useful, which motivates the estimability hypothesis
developed there. The \emph{effective dimension}
\begin{equation}
...
distributional at its kinks, so it is unsuitable for the pointwise autograd Laplacian used
here. Less obviously, being $C^\infty$ does not by itself determine how useful an activation
is for a Laplacian objective. What matters heuristically is also how its higher derivatives
are distributed over the input range. When the second and third derivatives are sharply
localised, curvature information can become concentrated in narrow regions of a deep
composition. Tanh and the logistic sigmoid are perfectly smooth, yet their higher
derivatives are sharply peaked and change sign. We use
$\mathrm{SiLU}(x)=x\,\sigma(x)$, whose higher derivatives are distributed more broadly.
Figure~\ref{fig:activation_derivatives} compares several smooth alternatives, including GELU
and Mish. This is a design heuristic
...
\begin{figure}[htbp]
  ...
  when such units are composed through depth. ReLU and SiLU are not shown; SiLU is the
  activation used in the reported models.}
...
message-passing CTNN. Chapter~\ref{ch:kernel} compares these implementations through the
geometry of the tangent kernel introduced in \S\ref{sec:theory-tangent-kernel}. Because the
archived models also differ in capacity, features and coordinate handling, the comparison
is descriptive rather than an isolated test of the relational channel.
...
decades~\cite{Lagaris1998-ANNforODEPDE}. Raissi, Perdikaris and Karniadakis consolidated and
popularised the modern PINN formulation in the automatic-differentiation
era~\cite{raissi2019pinns}, in which derivatives such as $\nabla u_\theta$ and
$\Delta u_\theta$ can be evaluated directly and used to train on the residual.
...
which is what bounds the MCMC-free update programme of Chapter~\ref{ch:kernel}.
...
The last of these determined the design of the collocation programme. Near a generic simple
node, $E_L$ has a pole, and differentiating a strong residual through it produces a
heavy-tailed gradient. The tested strong-form implementations stalled at the accuracy floor
documented in Appendix~\ref{app:catch22}. The weak form instead keeps the local energy as a
detached reward, which removes that derivative and proved substantially more stable.
