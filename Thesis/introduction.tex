...
$(1,5)$ and $(3,9)$ and mapped the radial and orientational development of the Wigner regime
\cite{ReimannManninen2002-QDReview,Egger_1999,Kong_2002,schweigert1994spectralpropertieschargedparticles,manninen2007metalclustersquantumdots,Cavaliere2009-WignerTransport,Ghosal2007-WignerMolecules}.
...
quantitative, null-calibrated and topology-conditioned diagnostics remain less common, and
it is not well understood how modern neural ans\"atze
...
variational Monte Carlo. This thesis belongs to the second tradition and borrows a residual
objective from the first: that objective serves as the pretraining stage of the variational
method and, in weak form, as the collocation route with MCMC-free gradient updates in
Chapter~\ref{ch:kernel}, a training method in its own right.

Carleo and Troyer introduced the modern neural-quantum-state formulation for lattice
systems~\cite{Carleo2017-NQS}. For continuum fermions the idea has
...
network's inputs are smooth, scaled to the trap and soft at short range. At a generic simple
node of an approximate state, the local energy has a pole; a strong-form objective that
differentiates through it can therefore produce heavy-tailed gradients for backflow
parameters that move the node. None of the variants tested in Appendix~\ref{app:catch22}
closed the resulting accuracy gap, which motivated the weak-form route. Beyond these,
...
update information through explicit edge states before a quantity is read out. In the
implemented comparison the two correlator families reach the same energy to four figures,
although their learned pair features are not otherwise identical. The energy therefore does
not resolve their difference, which has to be sought in other quantities.
...
\begin{enumerate}
  \item \textbf{Architecture.} How do the trained message-passing and separable
        implementations differ in the correlator and the backflow, and which differences
        survive state-level diagnostics?
  ...
  \item \textbf{Paradigm.} Removing the Markov chain from the gradient makes the role of
        the proposal explicit. Can an accurate wavefunction be trained by importance-sampled
...
interpreted. Chapter~\ref{ch:results} presents the energy comparisons and reads the physics from
...
diagnostics for the MCMC-free update route. The structural diagnostics act only on
configuration samples and can therefore be transferred to other many-body distributions;
the tangent diagnostics also require derivatives of the parametrised state.
Chapter~\ref{ch:analysis} defines them.
...
\begin{enumerate}
  ...
  \item A comparison of trained correlator and backflow implementations beyond their energy.
        At equal energy the message-passing correlator has lower local-energy variance and a
        higher fit to a selected operator dictionary; under the parametrisations used here it
        also concentrates its Fisher weight in fewer effective directions. The backflow sweep
        shows a rank-one collapse in the legacy conventional path, but capacity, input scaling
        and centre-of-mass handling prevent an architecture-only interpretation.
  ...
  \item Training with MCMC-free gradient updates by importance-sampled collocation, mapped to its limits:
        ...
        fifteen- to thirtyfold over a fixed one; and a cascade that reaches a stable checkpoint
        at $\omega=0.0035$, below which it no longer survives the step to a weaker trap.
  \item Topology-resolved diagnostics of the Wigner crossover, which quantify, shell by
        ...
        dots~\cite{Egger_1999,Kong_2002,schweigert1994spectralpropertieschargedparticles,Filinov_2001,Cavaliere2009-WignerTransport,Ghosal2007-WignerMolecules}.
\end{enumerate}
...
architecture, optimisation and sampling together with the MCMC-free update
