...
\qquad (\hbar=m_e=e=1).
\end{equation}
Spin does not appear in $H$, so eigenstates may be classified by $L_z$, $S$ and $S_z$.
The calculations below fix the populations in the two spin sectors, and hence $S_z$, without
an explicit projection onto total spin $S$.
...
\begin{align}
  \Psi_{\theta,\beta}(R)
  &= \mathrm{SD}_{\omega}\big(R'\big)\;
     \exp\!\Big(
       \sum_{i<j} u_{\sigma_i \sigma_j}(r_{ij})
       + W_\theta(R)
     \Big),\label{eq:ansatz-overall} \\
  R' &= R + \Delta_\beta(R).\label{eq:bf-coords}
\end{align}
Here $\mathrm{SD}_{\omega}$ evaluates the harmonic-oscillator determinant at physical
coordinates, $u$ hard-wires the Kato cusp, $W_\theta$ learns smooth medium- and long-range
structure, and $\Delta_\beta$ adjusts the nodes. The modules receive $R$ and perform any
trap scaling internally; the correlator is evaluated at the original coordinates, not at
$R'$. Per-particle displacements $\Delta_{\beta,i}$ are written $\Delta\mathbf{x}_i$ in the
results chapters.
...
(ii) \emph{Condition the geometry:} use trap scaling where stated and soft-core pair features so that first/second derivatives are bounded.
...
\label{subsec:nqs}
As with the backflow, the correlator comes in two forms that are compared in
\S\ref{sec:kernel-q1}: a \emph{separable} network that encodes particle and pair features
independently before pooling, and a copresheaf \emph{message-passing} (CTNN) network. Both
include the same analytic cusp, but their learned pair features are not identical; the
comparison is therefore between the implemented model families, not a one-variable ablation.
...
The Euclidean norm is nondifferentiable at exact coalescence and its Hessian is singular
there, which destabilises Laplacian evaluations. We therefore build \emph{safe} pair features
whose first and second radial derivatives remain bounded as $\tilde r\to 0$.
...
Equations~\eqref{eq:safe-feats} describe the intended dimensionless construction. In the
archived separable implementation, however, the distance is first converted to trap units
and the feature and gate scales are then multiplied by $a_{\rm ho}$. The effective feature
scale therefore changes with $\omega$. The reported sweep retains this implementation and is
interpreted with that limitation in Chapter~\ref{ch:kernel}.
...
The message-passing correlator keeps the analytic cusp but uses its own node and edge
embeddings. Its defining change is the copresheaf transport of
\S\ref{sec:ctnn-theory}: at each
...
$W_\theta(R)$. The Slater reference, cusp and training protocol are shared with the separable
model, but the CTNN uses relative vectors, their norm and square, together with a separate
raw-distance readout, rather than the six features and gate above. The comparison in
\S\ref{sec:kernel-q1} therefore measures the two implemented correlator families rather than
isolating message routing alone.

\section{Backflow}
\label{subsec:backflow-method}

Backflow deforms the physical coordinates used inside the Slater reference so that the nodal
surface can adapt to interparticle correlations:
\begin{equation}
  R' \;=\; R + \Delta_\beta(R) ,
  \qquad
  \Psi_{\theta,\beta}(R) \;=\; \mathrm{SD}_{\omega}(R')\;
  \exp\!\Big(\sum_{i<j} u_{\sigma_i\sigma_j}(r_{ij}) + W_\theta(R)\Big).
  \label{eq:bf-def}
\end{equation}
The correlator $W_\theta$ and cusp $u$ are evaluated on $R$; only the determinant sees $R'$.
...
is not a symmetry of the trap in any case; the CTNN's zero-mean projection is a regulariser
that removes a spurious centre-of-mass drift, not an imposed physical symmetry.

We study two archived backflow architectures. Both map a configuration to a bounded,
per-particle displacement field and use the same caller, which adds that field directly to
$R$. They do not use the same preprocessing or centre-of-mass constraint. The conventional
network is the baseline in Chapter~\ref{ch:kernel}; the copresheaf \textsc{CTNN} is the
production ansatz. Their comparison is therefore descriptive of these implementations, not
a controlled architecture ablation.
...
\begin{equation}
  \mathbf m_{ij}
  = \phi_\beta\!\big(
      \mathbf x_i,\ \mathbf x_j,\ \mathbf r_{ij},\
      r_{ij}^{\rm soft},\ (r_{ij}^{\rm soft})^2
    \big)\cdot w_{ij}^{\rm spin},
  \qquad r_{ij}^{\rm soft}=\sqrt{r_{ij}^2+\varepsilon_{\rm bf}^2},
  \label{eq:bf-message}
\end{equation}
...
\begin{equation}
  ...
  \qquad
  \delta\mathbf x_i = \psi_\beta\!\big(\mathbf x_i \,\|\, \mathbf m_i\big),
...
\label{sec:bf-init}
Both variants bound their output and start at the Slater nodes, but their coordinate
conventions differ:
\begin{equation}
  \Delta_\beta^{\rm conv}(R)=s_\beta\tanh[\delta X_{\rm conv}(R)],\qquad
  \Delta_\beta^{\rm CTNN}(R)=s_\beta P_{\rm COM}
  \tanh[\delta X_{\rm CTNN}(\sqrt{\omega}R)],
  \label{eq:bf-bound-scale}
\end{equation}
where $P_{\rm COM}$ subtracts the particlewise mean. The common caller adds both outputs
directly to the physical coordinates $R$. Their learned scales therefore do not have a
guaranteed common displacement unit. The final layer is zero-initialised and
$s_\beta=\mathrm{softplus}(s_\beta^{\rm raw})$ begins in $[0.02,0.10]$.

Near coalescence derivatives of the pair norm can make $\nabla\!\cdot\!\Delta_\beta$ spiky.
The conventional path mollifies $r_{ij}$ as in \eqref{eq:bf-message}; the CTNN also permits
an optional short-range gate in its internal trap units,
\begin{equation}
  \chi_{\rm bf}(\tilde r_{ij})=\frac{\tilde r_{ij}^2}{\tilde r_{ij}^2+r_g^2},
  \qquad \chi_{\rm bf}(0)=\chi'_{\rm bf}(0)=0,\quad r_g\approx 0.3,
  \label{eq:bf-gate}
\end{equation}
or use degree-normalised radial attention. The numerical mollifier
$\varepsilon_{\rm bf}=10^{-6}$ is fixed rather than tuned; the optional hard gate was disabled
in the reported architecture diagnostics unless stated otherwise. Its dormant extra
trap-scaling factor was removed after the reported runs; because the gate was disabled, this
code correction does not change the results.

Permutation symmetry is automatic. In the CTNN path a spurious centre-of-mass drift is
removed by
\begin{equation}
  \Delta_\beta \;\leftarrow\; \Delta_\beta \;-\; \frac{1}{N}\sum_{i=1}^N \Delta_{\beta,i},
  \label{eq:bf-com}
\end{equation}
taken over particles within each configuration. The conventional path returns its raw field;
the caller does not project it. This difference changes both the shifted determinant
coordinates and the ceiling of the rank diagnostic in \S\ref{sec:kernel-q1b}. Together with
the input-scale difference in Eq.~\eqref{eq:bf-bound-scale}, it prevents the archived sweep
from isolating backflow architecture.

Because only the determinant is evaluated at $R'$, backflow moves the \emph{nodes}
while the correlator exponent stays on $R$, and no Jacobian term arises. This
...
The correlator study includes a capacity-matched pair and a separable-network ladder from
$20$k to $164$k parameters against a fixed $80$k message-passing model. Capacity is therefore
tested directly, but the learned feature sets remain unmatched as noted above.
...
matched backflow pair was trained. Its preprocessing and centre-of-mass constraint also
differ, so the sweep reports the behaviour of two production implementations rather than an
...
hidden dimensions; ``rounds'' are message-passing steps ($n_\downarrow{+}n_\uparrow$ for the
  V-cycle). The main correlator pair differs in size, so \S\ref{sec:kernel-q1a} also reports
  the matched $89.4$k separable network and the full $20$k--$164$k ladder. The backflow
  variants are not matched in capacity or preprocessing. Settings follow the accompanying code
...
CG-SR with Tikhonov damping $\lambda$, a parameter-step cap and CG restarts. Collocation batch
...
\begin{enumerate}
  \item \textbf{Residual–based pretraining (stage I).} We fit the residual of $E_L$ to a moving scalar target $E_{\rm eff}$ under the current collocation proposal $q_t$,
  \(
    \mathcal{L}_{\rm res}(\theta)=\mathbb{E}_{R\sim q_t}\!\big[(E_L(R)-E_{\rm eff})^2\big].
  \)
  We begin with a \emph{self–target} (stabiliser) $E_{\rm eff}=\mu$ where $\mu=\mathbb{E}[E_L]$ over the current batch. This minimises the \emph{sample} variance of $E_L$ under the current collocation measure---which is not the same object as $\Var_{|\Psi|^2}(E_L)$ unless the two measures coincide---and avoids imposing an external energy target at the start. We then anneal toward a trusted reference $E_{\rm DMC}$ via
  \begin{equation}
  ...
  \item \textbf{Stochastic Reconfiguration (stage II).} After residual pretraining, we refine with SR (natural–gradient VMC). Writing $\bar O(R)=\partial_\theta \log|\Psi_\theta(R)|-\mathbb{E}_\pi[\partial_\theta\log|\Psi_\theta|]$ for the centred score,
  \begin{equation}
  ...
  \end{equation}
  using (preconditioned) conjugate gradients with restarts, a cap on the final parameter step, and batchwise centering/whitening of $O$.
\end{enumerate}
This pipeline uses residual pretraining to shape $W_\theta$ and, where present, the backflow
before applying the geometry-aware SR update.

\section{Configuration–space sampling}
\label{sec:sampling}

\paragraph{Guiding principle (permutation invariance).}
Within each spin sector, relabelling particles does not change the state. The sampler can
therefore target geometric motifs---clusters, shells, dimers and long tails---rather than
fixed particle identities. We also permute labels within spin sectors and rotate configurations
randomly to improve coverage without favouring an orientation.
...
\paragraph{Robustified updates.}
We remove the most extreme local-energy values by quantile (default $3\%$) before forming
an update. This reduces variance but changes the finite-sample objective. We also micro-batch
the evaluation and clip parameter gradients.
...
= \frac{1}{B}\sum_{b=1}^B \big(E_L(R_b)-E_{\rm eff}(\alpha)\big)^2,
\qquad
E_{\rm eff}(\alpha)=\alpha\,E_{\rm DMC}+(1-\alpha)\,\mu.
\end{equation}
For draws from $q_t$, this is an expectation under $q_t$. Written as a squared
Schrödinger residual, it carries the weight $q_t(R)/|\Psi_\theta(R)|^2$; it is not the
constant-weight residual analysed schematically in Appendix~\ref{app:laplacian}.
\begin{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Step cap.} The final update is scaled to satisfy a maximum Euclidean parameter-step norm.
  \item \textbf{Damping.} A small $\lambda$ stabilises low--variance directions.
\end{itemize}
The damping and step cap limit unstable finite-sample updates. They do not guarantee
well-behaved statistics when node-moving scores develop heavy tails; Appendix~\ref{app:catch22}
examines that limitation.
...
\begin{itemize}
  ...
  \item \textbf{CG-SR}: the same step solved matrix-free by preconditioned conjugate
    gradients, with Tikhonov damping and a parameter-step cap. It is most useful where $S$ is
...
Three ansatz families are trained under this objective in Chapter~\ref{ch:kernel}. The
Jastrow-only and backflow variants use the Slater--Jastrow core
$\Psi(\mathbf R)=\det[\phi_i(\mathbf r_j+\Delta\mathbf r_j)]\,e^{J(\mathbf R)}$; the third
replaces the determinant with a neural Pfaffian. They are smaller than the production networks of
\S\ref{sec:hyperparameters} to reduce the cost of the collocation campaign. Comparisons with
the production models are therefore not capacity matched.
...
The wavefunction evaluator receives physical coordinates. The sampler expresses Gaussian
widths and shell radii through $a_{\rm ho}=1/\sqrt{\omega}$ so that its components cover
comparable dimensionless regimes over $\omega\in[0.01,1]$. The CTNN feature paths also
scale their coordinates internally. The archived separable correlator and conventional
backflow do not follow the same convention, as documented above; results comparing those
implementations are therefore not treated as scale-controlled architecture tests.

Proposal candidates are drawn independently, so the batch carries no serial Markov-chain
autocorrelation. The retained batch is not i.i.d.\ in the strict sense, because multinomial
resampling can repeat candidates and the hard-example buffer reinjects earlier ones. The
design removes a mixing time, not all statistical dependence.

\section{The two workflows}
\label{sec:opt-summary}

Both routes use the same ansatz form and end at an energy evaluated under
...
For quantities that are \emph{nonlinear} functions of sample statistics (participation ratios, effective ranks, PCA cosines, and the coupling diagnostics defined in \S\ref{subsec:repr-bf} and reported in Chapter~\ref{ch:kernel}) a blocked standard error is not available in closed form, and we
...
\begin{equation}
g(r_\alpha)\;=\;\frac{2A_{\mathrm{eff}}}{N(N-1)}\;
...
\paragraph{Lab-frame and cross-shell order.}
For a frame with shells $s=1,\dots,S$ and occupancies $n_s$, the lab-frame statistic
\begin{equation}
  C_{\rm global} \;=\; \Big|\;\Big\langle \tfrac{1}{S'}\textstyle\sum_{s:\,n_s\ge3}
  \Phi_{n_s}\Big\rangle_{\rm frames}\Big| ,
  \label{eq:Cglobal}
\end{equation}
is the modulus of the frame-averaged complex order parameter. A freely rotating molecule
therefore has $C_{\rm global}\to0$ even when it is internally ordered: this statistic measures
pinning to the laboratory frame.

Cross-shell locking requires a branch-invariant phase comparison. For shells of occupancies
$m$ and $n$, let $z_m=\Phi_m/|\Phi_m|$, $z_n=\Phi_n/|\Phi_n|$ and
$g=\gcd(m,n)$. Then
\begin{equation}
  C_{m:n}=\left|\left\langle z_m^{\,n/g}(z_n^*)^{\,m/g}\right\rangle_{\rm frames}\right|
  \label{eq:Ctilde}
\end{equation}
is unchanged by a rigid rotation and contains no choice of phase branch. The earlier
$N{=}20$ cross-shell values were produced by an analysis script that was not retained and
used a phase-registration convention that cannot now be confirmed. They are therefore not
used quantitatively; the shellwise moduli $|\Phi_{n_s}|$ and angular-spacing statistics are
unaffected.
...
\paragraph{A selected operator dictionary.}
...
\begin{aligned}
  \mathcal{D}_1 &= \textstyle\sum_i \tilde r_i^{\,2}
    &&\text{breathing / monopole, proportional to $\partial H/\partial\omega$;}\\
...
Here $D^+$ is computed by a tolerance-controlled singular-value decomposition. The
dictionary is a \emph{chosen, non-exhaustive} probe, and nothing here establishes that
...
Chapter~\ref{ch:kernel} uses it. Because no random, dimension-matched dictionary or held-out
fit was run, the numerical $R^2$ values should not be read as an absolute test of physical
interpretability.
...
\emph{modulus} overlap, $\langle|\Psi_B/\Psi_A|\rangle\langle|\Psi_A/\Psi_B|\rangle$. It is
an upper bound on signed agreement and does not certify nodal agreement, because independently
trained backflows can move configurations across different nodes. We therefore use it only as
an amplitude-overlap diagnostic. Energy and $\Var(E_L)$ are properties of the state and are unaffected
