...
\cite{Hogberget2013-DMC,Nordhagen2023-NeuralNetworksFermions} and compiled DMC values, with
analytic anchors where available, for $N{=}2$~\cite{Taut1994-TwoElectrons,Hogberget2013-DMC}. We
...
\S\ref{subsec:separable-correlator} at ${\approx}54$k parameters and use different backflow
implementations: a \emph{conventional} single round of pairwise messages aggregated per particle
...
${\approx}236$k). They are named \emph{conv.\ BF} and \emph{MP BF}. Besides capacity, the
archived paths differ in coordinate preprocessing and centre-of-mass projection
(\S\ref{sec:bf-init}), so their comparison is not an architecture-only ablation. The
...
relative terms} (Tables~\ref{tab:Residual} and \ref{tab:energies},
Fig.~\ref{fig:rel_error_vs_omega}). All tabulated deviations are below $0.05\%$; the available
runs do not resolve a trend with particle number or confinement.
...
as an instrument in \S\ref{sec:wigner-molecule}, although it does not validate every property
of the wavefunction.
...
\begin{table}[htbp]
  ...
  \begin{threeparttable}
    ...
    \begin{tablenotes}\footnotesize
      \item[] Reference-energy provenance: values marked $^{\mathrm{H}}$ are H{\o}gberget's
      fixed-node diffusion Monte Carlo energies~\cite{Hogberget2013-DMC}, as tabulated in the
      published compilation of Nordhagen \emph{et al.}~\cite{Nordhagen2023-NeuralNetworksFermions}.
      The same values are reproduced by Haas~\cite{HaasHFQD}, whose compilation was used
      during development. The $N{=}2$ values come
      ...
      erratum~\cite{Pederiva2003-QD-DMC-Erratum}), cited here for the method, not for these
      particular numbers. Rows marked \emph{prediction} have no reference value to compare with.
    \end{tablenotes}
  \end{threeparttable}
\end{table}
...
The energy partition supports two conclusions.
...
$V_{\rm int}\approx2V_{\rm trap}$ to within $5\%$ (Fig.~\ref{fig:wigner_ratios}b). This ratio
approaches one over the sampled grid. By the virial relation it is linked to
$T/V_{\rm int}$, so it is a consistency view rather than an independent diagnostic.
...
Concretely, a Wigner molecule's internal angular order must be measured through
rotation-invariant relative quantities. For a shell $s$ with $n_s$ particles we compute the
bond-orientational modulus $|\Phi_{n_s}|$ and a corresponding
...
Recombining the inner--inner, inner--outer and outer--outer histograms with the combinatorial
weights of the ideal $(3,9)$ topology reproduces the global $g(r)$ almost exactly. Because
the component and total histograms use the same frames, this checks the bookkeeping rather
than the shell model itself. We therefore use a held-out generative test
(Table~\ref{tab:shell-model-fit-main}).
...
$\Delta\cos$, adds information about angular order beyond shell radii, and it grows
...
partition; the size of $\Delta\cos$ is model-dependent, its sign and its trend are not. The
candidate topology and radial partition were selected in the preceding full-data analysis,
so the split tests conditional reconstruction rather than fully held-out model selection.
...
\begin{table}[htbp]
\centering
\caption[Held-out shell-model fit against an angle-randomised null]{Conditional held-out
shell reconstruction. Radii, widths and angular jitter are fitted on half the frames and
scored on the other half. $\cos_{\rm poly}$ and $\cos_{\rm uni}$ compare the polygon and
angle-randomised models with the empirical pair-distance density; $\Delta\cos$ is their
difference. Intervals are $95\%$ block-bootstrap intervals. The topology and radial
partition were selected on the full data; sensitivity checks are in
Appendix~\ref{app:structural-tables:mixfit}.}
...
Angular-spacing disorder on the two occupied outer shells is low:
$\lambda_{7}\approx0.35$ and $\lambda_{12}\approx0.27$.
...
Wigner \emph{topology}, because it is accompanied by separated shells, shellwise angular
...
Six diagnostics, applied at four particle numbers and five confinements, together show the
crossover. As $\omega$
...
separate and generally stiffen, although several inner-shell width estimates are
nonmonotonic. Shellwise bond order stays within $1\%$ of its random-angle null down to
...
${\sim}40$ for $N{=}12$ at the weakest trap. None of these was built into the ansatz. At
$N{=}12$, $(3,9)$ is modal while $(1,11)$ and $(2,10)$ remain appreciably populated. This
sector mixing is consistent with the rotating-Wigner-molecule picture
~\cite{manninen2007metalclustersquantumdots,Filinov_2001}. At $N{=}20$ the
...
reorganisation more finely than within the decade. Neither the $(1,7,12)$ configuration nor
the broad separation of radial and angular ordering is new
~\cite{Filinov_2001,Cavaliere2009-WignerTransport,Ghosal2007-WignerMolecules}. The specific
contribution here is the combination of sector-conditioned bond-order and angular-spacing
statistics, finite-$n$ null calibration, topology-resolved measurements across the selected
grid, and a held-out polygon reconstruction of $g(r)$ against an angle-randomised baseline.
...
radial classifier with shellwise angular order: $(1,5)$, $(3,9)$, $(1,7,12)$ &
...
MCMC-free gradient training far into the correlated regime
