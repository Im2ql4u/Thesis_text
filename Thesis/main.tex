...
% UiO's official master's-thesis front page (Dept. of Informatics, v1.10). The package
% defaults to Norwegian, hence UKenglish; it also sets the sans-serif family to Helvetica,
% which this document does not otherwise use.
\usepackage[UKenglish]{uiomasterfp}
% The cover keeps UiO's Helvetica; the inner title page is set in the thesis's own font.
\makeatletter
\let\uiomfp@origpageiii\uiomfp@page@iii
\renewcommand{\uiomfp@page@iii}{{\normalfont\uiomfp@origpageiii}}
\makeatother
...
\begin{document}
% color=pink is UiO's profile red (#FB6666 / #FEE0E0); the date defaults to the current term.
% The explicit breaks keep "Dots" from standing alone on the last line at 32 pt.
\uiomasterfp[color=pink, dept={Department of Physics},
             program={Computational Science: Physics},
             supervisor={Morten Hjorth-Jensen},
             title={Physics-Informed Neural\\ Quantum States for\\ Quantum Dots}]


\include{abstract}
\thesisepigraph{To go wrong in one's own way is better than to go right in
someone else's.}{Fyodor Dostoevsky}{\textit{Crime and Punishment}}
\include{preface}
\tableofcontents
\listoffigures
\listoftables
