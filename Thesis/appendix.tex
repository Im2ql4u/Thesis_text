...
\paragraph{Relation to the objectives used in this thesis.}
Stage~I pretraining (\S\ref{sec:residual-stage}) draws configurations from an adaptive
proposal $q_t$ and minimises an empirical version of
\begin{align}
\mathcal L_{\mathrm I}(\theta)
&=\int q_t(\mathbf R)\,|E_L(\mathbf R)-E_{\rm eff}|^2\,d\mathbf R \\
&=\int |(\hat H-E_{\rm eff})\Psi_\theta|^2
\frac{q_t(\mathbf R)}{|\Psi_\theta(\mathbf R)|^2}\,d\mathbf R,
\label{eq:lap:stage1-weight}
\end{align}
where the second line holds away from nodes. It therefore induces the weight
$w_{\theta,t}=q_t/|\Psi_\theta|^2$, rather than the constant weight used in the simplest
form of Eq.~\eqref{eq:lap:resloss}. Because this weight changes with both the proposal and
the wavefunction, and because the implementation reuses samples and trims outliers, the
fixed-weight analysis below is not the exact Stage~I Hessian. It isolates the more general
curvature amplification caused by squaring a residual that contains the Laplacian.

Stage~II instead minimises the variational energy (\S\ref{sec:sr-stage}), with updates
preconditioned by the sampled quantum geometric tensor. The later collocation route
(\S\ref{sec:method-collocation}) uses a weak, score-function gradient and does not
differentiate through the local energy. Neither objective has the normal-equation geometry
derived below, although both still require the Laplacian when evaluating $E_L$.
...
This appendix examines a difficulty encountered when backflow was trained with a
\emph{strong-form} collocation loss: the pointwise variance of the local energy $E_L$.
Near a generic simple node of an approximate wavefunction, $E_L$ has a pole. A smooth
collocation proposal therefore overweights precisely the region in which the residual is
singular, while screening that region changes the objective and may remove configurations
that are especially sensitive to motion of the node. Across the interventions tested for
the $N{=}6$, $\omega{=}0.5$ dot, this pipeline did not improve beyond an error of about
$0.3\%$.

The local analysis explains why the strong form is difficult, but it does not prove that
the observed floor is irreducible or that nodal singularities uniquely caused every failed
run. The experiments also use smaller models than the production VMC calculation. The
weak-form collocation method used in the main results avoids differentiating through $E_L$
and performs substantially better. This appendix records the strong-form result and the
reason for changing the objective.
...
\paragraph{For an approximate wavefunction, the cancellation need not hold.}
Let $d_\theta$ be signed distance from a simple node of the trial wavefunction.
Locally, $\Psi_\theta\sim a d_\theta$, whereas $\hat H\Psi_\theta$ is generically
nonzero on that surface. The local energy then behaves as
\begin{equation}
E_L(\mathbf R)
=\frac{\hat H\Psi_\theta}{\Psi_\theta}
\sim \frac{c}{d_\theta}
\qquad\text{as }d_\theta\to 0,
\label{eq:c22:EL_div}
\end{equation}
with a coefficient $c$ that vanishes only when the required cancellation happens.
Equation~\eqref{eq:c22:EL_decomp} shows how the pole can arise through the determinant
terms, but no comparison with the unknown exact nodal surface is needed: it is a local
property of the trial wavefunction and its residual.


\section{Local power counting near a trial node}
\label{app:catch22:mismatch}

The sampling measure determines whether the pole in Eq.~\eqref{eq:c22:EL_div} is
integrable. In local normal coordinates, a tubular neighbourhood of a codimension-one
nodal surface has volume element $d\sigma\,dd_\theta$. For a simple node,
\begin{equation}
|\Psi_\theta|^2\sim d_\theta^2,
\qquad
E_L^2\sim d_\theta^{-2}.
\label{eq:c22:scaling}
\end{equation}

Under the VMC measure $p_\theta\propto|\Psi_\theta|^2$, the local contribution to
the second moment of $E_L$ is therefore finite:
\begin{equation}
\int_{-\varepsilon}^{\varepsilon}p_\theta E_L^2\,dd_\theta
\sim\int_{-\varepsilon}^{\varepsilon}dd_\theta<\infty.
\label{eq:c22:integrand}
\end{equation}
The same second moment under a proposal with a smooth, nonzero density at the node
scales as $\int d_\theta^{-2}dd_\theta$ and is not locally integrable. Directly
differentiating the empirical strong loss is more singular still. For a parameter that
moves the node, $\partial_\theta d_\theta\neq0$ and
\begin{equation}
\partial_\theta E_L
\sim \frac{\partial_\theta c}{d_\theta}
-\frac{c\,\partial_\theta d_\theta}{d_\theta^2}.
\label{eq:c22:grad_EL_local}
\end{equation}
In the determinant representation, the same behaviour appears in terms such as
\begin{equation}
...
\end{equation}
which diverge as $D\to0$. Finite batches regularise these expressions through their
nearest sampled distance to the node, but the resulting updates can be dominated by rare
points. This predicts severe heavy tails, not the impossibility of optimisation in every
strong-form implementation.


\section{The catch-22}
\label{app:catch22:statement}

The preceding analysis gives the practical tension behind the name of this appendix:

\begin{enumerate}
\item \textbf{Backflow moves the node.}
  Parameters that displace a simple node have a logarithmic derivative
  $\partial_{\theta_{\mathrm{BF}}}\log|\Psi|\sim d_\theta^{-1}$, so
  near-node configurations are unusually sensitive to those parameters.

\item \textbf{The strong residual is singular at a generic trial node.}
  As shown in \S\ref{app:catch22:divergence}--\ref{app:catch22:mismatch},
  $E_L$ generally has a first-order pole, and differentiating it with respect
  to a node-moving parameter introduces a second-order pole.

\item \textbf{Screening controls the pole by suppressing its neighbourhood.}
  Under screened collocation, the top-$K$ selection ranks points by
  $|\Psi|^2/q\propto D^2 e^{2J}/q$.
  Near-node points therefore tend not to enter the training batch. This stabilises
  the estimator, but it also weakens its sensitivity to the part of the wavefunction
  that changes most sharply when the node moves.
\end{enumerate}

\noindent
A finite implementation must therefore balance two errors:
\begin{quote}
\emph{Including points arbitrarily close to a trial node exposes the strong-form
gradient to heavy tails. Screening them improves stability but changes the effective
objective and can reduce the signal for moving the node.}
\end{quote}
The experiments below are consistent with this account, but do not isolate it from
capacity, proposal design, or optimisation effects.


\section{Why VMC and the weak form are less singular}
\label{app:catch22:vmc}

VMC samples from $p_\theta\propto|\Psi_\theta|^2$. For a simple node, the
probability mass within normal distance $\epsilon$ scales as $\epsilon^3$, so configurations
closest to the node are suppressed without being removed entirely. The variational force
for a real wavefunction is
\begin{equation}
g_i=2\,\operatorname{Cov}_{p_\theta}[E_L,O_i],
\qquad O_i=\partial_{\theta_i}\log|\Psi_\theta|.
\label{eq:c22:vmc-force}
\end{equation}
For a parameter that moves the node, $E_L\sim d_\theta^{-1}$ and
$O_i\sim d_\theta^{-1}$. The per-sample product therefore scales as
$d_\theta^{-2}$. Its mean under $p_\theta\sim d_\theta^2$ is locally finite,
although its second moment need not be. VMC can thus retain a usable nodal signal while
remaining exposed to heavy-tailed finite-sample noise.

The decisive difference from the strong form is that the force in
Eq.~\eqref{eq:c22:vmc-force} does not differentiate $E_L$. The weak-form collocation
estimator used later has the same property: it treats $E_L$ as a detached reward and
differentiates the score. SR additionally preconditions the VMC force with the sampled
metric, but that does not remove the underlying tails. Robust trimming, damping, and the
step cap in \S\ref{sec:sr-stage} control their finite-sample effect.

None of the strong-form interventions in \S\ref{app:catch22:experiments} closed the
observed gap. This supports the change of objective, while leaving open whether a different
strong-form estimator or a fully matched experiment could do so.
...
\cite{Hogberget2013-DMC,Nordhagen2023-NeuralNetworksFermions}).
...
bounds what this objective achieves at this capacity rather than isolating the objective
alone. The interventions below did not move the floor, but the capacity difference prevents
us from attributing the full gap to the objective.
...
Neither importance-resampled nor stochastic top-$K$ selection improved the result.
Gumbel top-$K$, which replaces deterministic selection with stochastic
Gumbel--softmax, triggered early stopping after rapid energy divergence. These tests
alter both coverage and selection, but we did not record the resulting distribution of
distances to the node. They are therefore consistent with the heavy-tail account without
directly establishing that greater near-node coverage caused the failures.
...
$T$ clipping & $\operatorname{sign}(T)\min(|T|,T_{\max})$,
  $T_{\max}{=}30$ & 0.43 & neutral \\
\bottomrule
\end{tabular}
\end{table}
...
geometry.
Here it does not improve the energy. This is consistent with the problem arising partly
from the differentiated local energy (Appendices~\ref{app:laplacian}--\ref{app:coulomb}),
which parameter-space preconditioning does not remove. The experiment alone does not
separate that explanation from an inaccurate K-FAC approximation or poor tuning.
...
\midrule
\textbf{Production family, VMC\,+\,SR} & $<$\textbf{0.01} &
  \textbf{variational} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
\item Neither SIR nor Gumbel top-$K$ improves the error. Because nodal distances were
  not logged, these runs do not show whether a change in near-node coverage was responsible.
\item The interventions that changed the loss most directly (the hybrid mean-energy term
  and determinant filter) produced the largest errors. Because the collocation measure is
  not $|\Psi|^2$, their mean-energy terms are not variational; the experiments do not isolate
  this fact from the other changes made by those losses.
\item The smoothness penalty $\lambda\|\nabla^2\Delta\mathbf x\|^2$
  helps modestly (from $0.59\%$ to $0.38\%$ without it vs.\ with it).
  This is consistent with reduced curvature sensitivity, but does not identify which
  contribution to the local energy changed.
\item Among the differentiated-residual variants tested here, none approaches the
  production VMC\,+\,SR result. The later weak-form collocation method also trains
  backflow without differentiating $E_L$.
\end{itemize}
...
In the implemented strong objective
$\mathbb E_q[(E_L-E_{\rm eff})^2]$, these compound
...
Under the collocation loss, the gradient is determined by $E_L$ at the current collocation
points while the update itself moves the node. Reused points may therefore cease to describe
the most relevant neighbourhood after the update. VMC+SR instead refreshes samples from the
current $|\Psi_\theta|^2$, allowing its sampling distribution to track the moving node more
closely. This difference is consistent with the observed stability gap, but was not isolated
from model size or the other estimator choices.
...
the same question---what limits the MCMC-free update route---from the side of what does not
work.
...
and parameter-step caps. And the \emph{training style}: single sweeps, wave-based cascades with
checkpoint inheritance, and asynchronous campaigns relaunching from the best available
checkpoint.

Two patterns held across this unmatched search. Moderately sized backflow$+$Jastrow models
with stable training outperformed the larger variants tested here, without isolating whether
size, architecture or optimisation caused the difference. At low $\omega$, regularising the resampling
...
and $N{=}12$ at $\omega{=}1.0$ ($E=65.70699\pm0.00595$, $+0.010\%$). These are single
end-of-run evaluations of selected runs and were not independently re-evaluated; the
selection effect documented in \S\ref{sec:gradient-chain} makes the strong-confinement values
among them optimistic by several hundredths of a percent. For $N{=}2$, the raw Metropolis
deviations are $+0.035\%$, $-0.019\%$, $+0.035\%$, $+0.098\%$ and $+0.806\%$ at
$\omega=1,0.5,0.1,0.01,0.001$. The archived zero-variance regression intercepts give
$+0.012\%$, $+0.023\%$, $-0.004\%$, $+0.002\%$ and $+0.154\%$, respectively. Those
intercepts are not variational estimates, and no regression interval was retained, so they
are not used as the headline energies in Table~\ref{tab:collocation}.
...
was necessary to keep the search from circling. Stricter SR step caps, heavier
damping, ESS-adaptive sampling and rollback controls improved operational stability without
closing the accuracy gap at the hardest targets. In the tested $N{=}6$ low-$\omega$
campaign, changing the proposal was the intervention associated with closing that gap
...
The message-passing backflow has small displacements throughout this grid. Its $\tanh$
output bound limits their scale; the present data do not isolate any additional effect of
the message-passing structure. The conventional backflow can exhibit
...
Under the stated cutoff, the sampled displacement matrix is not dominated by one or two
collective directions such as breathing or rigid rotation. Its effective rank reaches the
nominal COM-free ceiling. Rank alone does not show that every particle depends on every
other particle.
...
something the present measurement can decide. Chapter~\ref{ch:kernel} compares the two
implemented backflow families, but their capacity, preprocessing and centre-of-mass
handling are not matched (\S\ref{sec:kernel-q1b}).
...
The correlator's measured feature geometry is therefore similar with and without the
displacement field on these samples. This is a statement about that geometry, not about the subspace the
...
%% ──────────────────────────────────────────────────────────────────────
\section{Correlator--backflow alignment}
\label{sec:repr-coupling}

The previous subsection found that the correlator feature geometry changes little when
backflow is present in this trained solution.
We next ask whether the displacement aligns with quantities derived from the correlator.
Three diagnostics address that narrower question:
...
\paragraph{An alignment change near $\omega\approx 0.1$ on the sampled grid.}
The data show their largest change between two adjacent sampled confinements.
...
Here the displacement is aligned with directions to which the correlator is especially
sensitive. This is compatible with cooperative refinement, but the diagnostic does not show
that it corrects a particular residual error.
The largest sampled change lies between $\omega=0.01$ and $\omega=0.1$,
...
nearest neighbour.
The displacement points away from the nearest neighbour more often than toward it, which is
consistent with a deepening of the correlation hole.
...
(isotropic). This is consistent with the larger equilibrium separations reducing the
directional correlation-hole signal.
...
one should expect. Deciding the question requires a matched experiment that varies only the
architecture.
...
$\alpha_{V_{\rm trap}}\simeq0.81,\,0.74,\,0.72$ for $N=2,6,12$. The kinetic energy therefore
...
three decades, against the measured $9.3$, $10.2$ and $11.4$.
...
placed at \emph{independent uniform} angles on a ring, $n\Phi_n=\sum_k e^{in\phi_k}$
is a two-dimensional random walk of $n$ unit steps, so
\begin{equation}
  \big\langle|\Phi_n|\big\rangle_{\rm random}=\frac1n\int_0^\infty\frac{1-J_0(t)^n}{t^2}\,dt
  \;\xrightarrow{\;n\to\infty\;}\;\frac{\sqrt\pi}{2\sqrt n}\approx\frac{0.886}{\sqrt n},
  \qquad
  \big\langle|\Phi_n|^2\big\rangle_{\rm random}=\frac1n .
  \label{eq:phi-null}
\end{equation}
The exact values at the occupancies that occur are $0.525$, $0.402$, $0.366$, $0.338$,
$0.298$, $0.269$, $0.257$ and $0.204$ for $n=3,5,6,7,9,11,12,19$; the Rayleigh limit
undershoots them by $0.4$--$2.6\%$, which is why it is not used here.
...
the right threshold. From $\omega=1$ down to $\omega=0.1$ the ratio is $0.99$--$1.01$ at
every particle number and on every ring. At $\omega=0.01$ it lifts, to $1.08$ at $N{=}6$ and
$1.02$ at $N{=}12$, while $N{=}20$ stays at $1.00$. Only at $\omega=10^{-3}$ does it become
large: $1.63$ on the $N{=}6$ ring, $1.16$ and $1.55$ on the two rings of the $N{=}12$ $(3,9)$
molecule, and $1.18$ and $1.46$ on the outer two rings of the $N{=}20$ $(1,7,12)$ molecule.

Whether the small departures are \emph{resolved} is a separate question from whether they
are \emph{large}, and here both can be answered. Because the shells are assigned by radial
rank alone and no angular cut is applied, Eq.~\eqref{eq:phi-null} is also the null of this
particular pipeline; generating it numerically instead---keeping every frame's radii and
redrawing all angles uniformly, then running the identical shell assignment---reproduces it
within its error in every cell. Block-bootstrap errors on the ratio are $0.002$--$0.007$. On
that scale the $N{=}6$ ring at strong confinement sits resolvably \emph{below} the null, at
$0.989\pm0.002$ for $\omega=1$ and $0.990\pm0.002$ for $\omega=0.5$. That is physics rather
than artefact. Since
$\langle|\Phi_n|^2\rangle=n^{-1}+n^{-2}\sum_{j\neq k}\langle\cos n(\theta_j-\theta_k)\rangle$,
a depletion of close angular pairs---a correlation hole along the ring---makes the cross
terms negative: what the ring shows at strong confinement is short-range angular repulsion
without $n$-fold order. At $N{=}12$ and $N{=}20$ the ratio stays within $0.5\%$ of the null
through $\omega=0.1$.

The conclusion that survives is the one that matters: statistical detectability is not what
separates the regimes here, magnitude is. On that criterion $n$-fold order is absent through
$\omega=0.1$, where the resolved departures are all within about one percent; weakly present at
$\omega=0.01$ and clearly so only at $N{=}6$; and unambiguous at $10^{-3}$, where every ring
exceeds the null by $16$--$63\%$.

The outer-shell relative widths generally decrease as the trap weakens, while several
inner-shell widths are nonmonotonic when the topology changes. On these diagnostics, radial
localisation is already visible before the large rise in $n$-fold bond order anticipated in
\S\ref{sec:wigner-markers-theory}.

The values in this table are for the \emph{modal} topology over all frames. The
sector-conditioned values quoted for $N{=}6$ in \S\ref{sec:wigner-angular} are computed only on
the frames assigned to a given sector, and in an earlier analysis pass whose inputs we could
not match to the runs used here. They are not uniformly higher, and the differences are not
small (Table~\ref{tab:bondorder-estimators}). At $\omega=0.01$ the excess over the null ranges
from $8\%$ on the modal estimator to $13\%$ and $27\%$ on the two sectors; at $10^{-3}$ the
modal value lies between the two sector values. The staging does not depend on the choice:
every estimator places $N{=}6$ modestly above the null at $\omega=0.01$ and far above it at
$10^{-3}$. The size of the $\omega=0.01$ excess does, and should be quoted as a range.

\begin{table}[H]
\centering
\caption[Two bond-order estimators at $N{=}6$]{Two estimators of the $N{=}6$ bond order,
as ratios to the exact random-angle null ($0.402$ for $n{=}5$, $0.366$ for $n{=}6$). The
modal estimator (this appendix) assigns every frame to $(1,5)$; the sector-conditioned
estimator (\S\ref{sec:wigner-angular}) splits the frames between $(1,5)$ and $(0,6)$ and comes
from an earlier analysis pass.}
\label{tab:bondorder-estimators}
\small
\begin{tabular}{c ccc}
\toprule
 & modal, all frames & \multicolumn{2}{c}{sector-conditioned} \\
\cmidrule(lr){3-4}
$\omega$ & $|\Phi_5|$ on $(1,5)$ & $|\Phi_5|$ on $(1,5)$ frames & $|\Phi_6|$ on $(0,6)$ frames \\
\midrule
$0.01$  & $1.08$ & $1.13$ ($69\%$ of frames) & $1.27$ ($31\%$) \\
$0.001$ & $1.63$ & $1.49$ ($80\%$) & $1.81$ ($20\%$) \\
\bottomrule
\end{tabular}
\end{table}
...
widths; $|\Phi_{n_s}|$
is the rotation-invariant bond-order modulus on shell $s$; and the final column is $|\Phi_{n_s}|$
divided by its exact random-angle expectation, Eq.~\eqref{eq:phi-null}. A value of $1.00$
in that column means the ring is equal to the independent-angle null within quoted
precision; see the text for why departures of about a percent are at the resolution of
this comparison. Single-particle inner shells are omitted from the
...
\midrule
 6 & $1.0$   & $(1,5)$     & $0.70,\,1.66$ & $0.43,\,0.32$ & $0.397$ & $0.99$ \\
 6 & $0.5$   & $(1,5)$     & $0.76,\,1.78$ & $0.42,\,0.31$ & $0.397$ & $0.99$ \\
 6 & $0.1$   & $(1,5)$     & $0.93,\,2.15$ & $0.41,\,0.28$ & $0.405$ & $1.01$ \\
 6 & $0.01$  & $(1,5)$     & $1.33,\,2.96$ & $0.42,\,0.22$ & $0.434$ & $1.08$ \\
 6 & $0.001$ & $(1,5)$     & $1.12,\,4.29$ & $0.63,\,0.15$ & $\mathbf{0.654}$ & $\mathbf{1.63}$ \\
\midrule
12 & $1.0$   & $(1,11)$    & $0.55,\,2.00$ & $0.47,\,0.34$ & $0.268$ & $1.00$ \\
12 & $0.5$   & $(1,11)$    & $0.60,\,2.18$ & $0.47,\,0.33$ & $0.268$ & $1.00$ \\
12 & $0.1$   & $(1,11)$    & $0.74,\,2.68$ & $0.45,\,0.31$ & $0.270$ & $1.00$ \\
12 & $0.01$  & $(1,11)$    & $1.04,\,3.77$ & $0.41,\,0.28$ & $0.275$ & $1.02$ \\
12 & $0.001$ & $(3,9)$     & $2.52,\,6.04$ & $0.61,\,0.24$ & $\mathbf{0.611,\,0.462}$ & $\mathbf{1.16,\,1.55}$ \\
\midrule
...
20 & $0.01$  & $(1,19)$    & $1.05,\,4.47$ & $0.49,\,0.32$ & $0.205$ & $1.00$ \\
20 & $0.001$ & $(1,7,12)$  & $1.27,\,4.19,\,7.83$ & $0.47,\,0.24,\,0.11$ & $\mathbf{0.398,\,0.375}$ & $\mathbf{1.18,\,1.46}$ \\
\bottomrule
\end{tabular}
\end{table}
...
radii alone. The split tests the fitted parameters conditionally: the shell topology and
broad analysis design were selected using the full data, so this is not held-out model
selection. Table~\ref{tab:shell-model-fit} reports both.
...
\paragraph{How firmly $\Delta\cos$ is determined.}
The analysis code, restored from the notebook that produced this table, reproduces every
published $\Delta\cos$ to six decimals, and two further tests were run on it. The first is a
block bootstrap: contiguous blocks of both the training and the held-out frames are
resampled, the shell model is refitted and a fresh model sample drawn in every replicate, so
the interval covers sampling noise in the data, uncertainty in the fitted parameters and
Monte Carlo noise in the model. Over $200$ replicates the $95\%$ intervals are
$0.0002$--$0.0036$ wide and exclude zero everywhere, including $N{=}20$ at $\omega=1$,
$[0.0027,0.0030]$. The growth with $1/\omega$ is resolved: at $N{=}6$ all five intervals are
disjoint, and at $N{=}12$ and $N{=}20$ only the two strongest confinements touch.

The second test varies the analysis choices. Halving or doubling the number of histogram
bins leaves $\Delta\cos$ unchanged at the fourth decimal, and swapping or interleaving the
training and held-out halves moves it by at most $0.0003$. The shell-detection threshold
matters only where it changes the partition: lowered far enough to split the single $N{=}20$
ring into a spurious $(1,3,16)$ at $\omega=0.5$ and $0.1$, or to give $(1,3,8)$ at $N{=}12$,
$\omega=0.01$, it lowers $\Delta\cos$ by $10$--$16\%$, and in that variant the $N{=}20$
value at $\omega=0.5$ drops just below the one at $\omega=1$. The largest dependence is on the
angular-jitter model: scaling the fitted jitter by $0.75$ or $1.25$ moves $\Delta\cos$ by up
to $+34\%$ and $-22\%$, but in both variants it stays positive and grows monotonically as the
trap weakens, at every particle number. The size of $\Delta\cos$ is therefore
model-dependent; its sign and its trend are not. The code, the replicates and every variant
are in \texttt{scripts/wigner\_null\_bootstrap.py} and
\texttt{results/analysis/2026-09-12\_wigner\_null\_bootstrap/}.
...
\chapter{The cost of a training step}
\label{app:walltime}

The main text argues that removing the Markov chain from the gradient changes what a training
step costs. This appendix times it. All numbers are wall-clock seconds per gradient step on one
NVIDIA RTX 2080 Ti in double precision, on a node whose eight GPUs were shared with other users
running at $67$--$100\%$ utilisation throughout; they include that contention, and ratios are
more reliable than absolute values. Networks are at initialisation, since the trained state
does not change what a step costs, and $\omega=1$. The grid of
Table~\ref{tab:walltime-grid} averages three steps after a discarded first one; the production
collocation epochs vary by about $2\%$ over eight epochs, and the production SR step is timed
after its burn-in call, where consecutive steps agree to $0.1\%$. The scripts are \texttt{scripts/bench\_production\_sr.py},
\texttt{bench\_walltime.py} and \texttt{bench\_score\_matrix.py}.

\section{The two production pipelines}

Table~\ref{tab:walltime-production} times the pipelines as they produced the results of this
thesis. The SR--VMC step is the production routine with the settings of the example run:
$1200$ persistent walkers and five 20-sweep Metropolis segments per step, for 100 sweeps per
walker and about $3.6\times10^3$ retained rows, with the exact Laplacian. Its phases are timed inside the
routine without changing it. The collocation epoch is the robust recipe of
\S\ref{sec:colloc-reliability}: $16\times4096$ candidates from the Gaussian mixture, weighted
by $|\Psi|^2/q$ and resampled to $4096$, the exact-Laplacian local energy as a REINFORCE reward,
and Adam. The two pipelines build their networks differently ($108$k parameters for the
SR--VMC step, the ${\sim}49$k collocation ansatz of \S\ref{subsec:colloc-families}), so this
table compares pipelines, not paradigms.

\begin{table}[H]
\centering
\caption[Wall time of the production pipelines]{Wall time per step of the two production
pipelines at $N{=}6$. The SR--VMC step's first call adds a one-off burn-in of $1000$ sweeps
(its chain then takes $89$\,s); that call is excluded here.}
\label{tab:walltime-production}
\small
\begin{tabular}{l rrrr}
\toprule
 & Markov chain & local energy & score matrix & total \\
\midrule
SR--VMC step ($100$ sweeps $\times$ $1200$ walkers) & $8.5$ & $12.2$ & $495.2$ & $516.4$ \\
collocation epoch ($16\times4096$ candidates) & --- & \multicolumn{2}{c}{(not separated)} & $21.5$ \\
\bottomrule
\end{tabular}
\end{table}

\noindent A production collocation epoch takes $3.1$\,s at $N{=}2$ and about $115$\,s at
$N{=}12$ on the same hardware.

The production SR--VMC step is twenty-four times slower, and the Markov chain accounts for
$1.6\%$ of it. Ninety-six percent is the score matrix $O$ that SR needs. The production routine
builds it one row at a time by backpropagating each sample's $\log|\Psi|$ through the graph of
the whole walker batch, so each row costs roughly a backward pass over the entire batch.

\section{Paradigm and optimiser, on one network}

To separate the main timing contributions, Table~\ref{tab:walltime-grid} runs three routes
on one network---VMC with Adam, collocation with Adam, and VMC with SR---and times two SR
implementations. All use a CTNN V-cycle correlator and CTNN backflow ($108$k parameters) with
the same exact-Laplacian local energy. VMC uses $3072$ walkers and $30$ sweeps per step,
collocation $4096$ points resampled from $16\times4096$ candidates, SR $1024$ samples. At
$N{=}20$ both paradigms use $1024$ configurations, because the shared cards did not have the
memory for more. The SR score matrix is built either by the per-sample loop of the thesis's
code or by a vectorised per-sample gradient (\texttt{torch.func}), which returns the same matrix
to a relative difference below $10^{-14}$.

\begin{table}[H]
\centering
\caption[Wall time by paradigm and optimiser]{Seconds per step on one network. Entries marked
$^{*}$ are composed from the Adam route's sampling and local-energy times and the separately
timed score matrix and solve; at $N{=}6$, where both were measured, composition and measurement
agree to $1\%$.}
\label{tab:walltime-grid}
\small
\begin{tabular}{c rr rr}
\toprule
 & \multicolumn{2}{c}{Adam} & \multicolumn{2}{c}{VMC with SR} \\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}
$N$ & VMC & collocation & per-sample loop & vectorised \\
\midrule
$2$  & $5.1$  & $4.2$  & $26.4$      & $7.6^{*}$  \\
$6$  & $16.3$ & $17.6$ & $62.1$      & $18.6^{*}$ \\
$12$ & $64.7$ & ---    & $185^{*}$   & $66^{*}$   \\
$20$ & $75.4$ & $67.0$ & $387^{*}$   & $79^{*}$   \\
\bottomrule
\end{tabular}
...
Three things follow. At the same optimiser the paradigm changes the cost of a step by about ten
percent: at $N{=}6$ the chain takes $5.5$\,s of a VMC step and the importance weights $3.5$\,s
of a collocation step, while the local energy takes $10$--$13$\,s of either. Collocation's
weights are not free, since every candidate needs $|\Psi|$ evaluated; per retained point they
cost half of what the chain costs here ($16$ evaluations against $30$ sweeps). The local energy
is $62$--$84\%$ of an Adam step from $N{=}6$ up, so a cheaper Laplacian would speed training more
than any change of sampler. And the SR overhead is almost entirely the per-sample loop: building
$O$ for $1024$ samples takes $19$, $46$, $121$ and $312$\,s at $N=2,6,12,20$ by the loop and
$2.0$, $1.8$, $2.4$ and $3.8$\,s vectorised, with the Gram matrix and its solve another
$1.2$\,s. Vectorised, SR costs $1.0$--$1.5$ times an Adam step.

\section{What per-step timing does not measure}

A step's wall time counts work, not information. The Markov chain's real cost is statistical:
the burn-in, paid once per run and again whenever the state moves far; the correlation between
successive samples, which makes a batch of walkers worth fewer independent points than its
size; and slow mixing between well-separated configurations, which is worst in the Wigner
regime. None of that appears above, and none of it applies to independently drawn candidates.
The measure that would capture it is time per \emph{independent} sample, the per-sweep cost
times the chain's integrated autocorrelation time on a trained weak-confinement state, which we
did not measure. Nor does the cost of a step say how many steps each route needs.

\chapter{Code, data and reproducibility}
\label{app:reproducibility}

\paragraph{Code.} The source code used for the ansatz, both training routes, the structural
and tangent-space diagnostics, the evaluation scripts and the figure scripts is public at
\url{https://github.com/Im2ql4u/Thesis}, together with retained run configurations, model
metadata and the derived tables from which the numbers in Chapters~\ref{ch:results}
and~\ref{ch:kernel} are quoted. The text alone, with the figures it includes, is at
\url{https://github.com/Im2ql4u/Thesis_text}.

\paragraph{Models and raw data.} Trained weights and the $|\Psi|^2$ sample bundles from
which every structural diagnostic is computed are too large for the repository. They are kept
on the University of Oslo machine-learning nodes where the calculations ran and are available
on request.

\paragraph{Seeds and initialisation.} Every run records its seed in its metadata, and seeds
are stated in the text wherever a claim rests on seed variation. The collocation runs are
warm-started, and two facts limit how far they can be repeated from the current code. The
canonical pretrained initialisation behind the continuation chains was lost in a repository
rewrite in June 2026, so those runs can be re-evaluated from their saved checkpoints but not
retrained bit for bit. And the Jastrow cusp convention changed in May 2026, from a fixed
decay length to the oscillator length; checkpoints trained before the change must be
evaluated with the convention they were trained with, which the evaluation script infers from
the checkpoint metadata.

\paragraph{The importance-sampling error and what was checked after it.} In March 2026 the
collocation sampler was found to return, for each proposal draw, the log-density of the
Gaussian component that generated it rather than the log-density of the whole mixture. The
resulting error in the log-weights grows with the dimension $Nd$ and with the spread of the
component widths: negligible at $N{=}2$, $\omega=1$, it reached a factor of order $e^{12}$ in
the importance weights at $N{=}6$, $\omega=0.01$, which is why its signature---loss of
accuracy at larger $N$ and weaker confinement---was indistinguishable from an architectural
limit. The error sat in the importance weights and therefore only in the collocation route;
the production energies of Chapter~\ref{ch:results} are evaluated by Metropolis sampling of
$|\Psi|^2$, which does not use those weights. After the correction (26 March 2026) three
things were checked. The collocation results reported here come from runs made after it, or,
for $N{=}12$, from continuation runs whose final training stage used the corrected weights.
Every $N{=}6$ and $N{=}12$ collocation energy in Tables~\ref{tab:collocation}
and~\ref{tab:reliability} was independently re-evaluated for this thesis by Metropolis
sampling, with four separately seeded chains per checkpoint and a blocking analysis
(\texttt{scripts/eval\_collocation\_blocked.py}). And the structural analysis, whose
original code had been lost with a deleted notebook, was recovered and restored
(\texttt{src/wigner\_structure.py}); it reproduces every published $\Delta\cos$ to six
decimals.

\paragraph{Known gaps.} Two quantities cannot be regenerated from the saved outputs: the
cross-shell registration statistic $\tilde C_{\rm global}$ at $N{=}20$
(\S\ref{sec:wigner-angular}), whose analysis code was not retained, and the $N{=}20$,
$\omega=0.01$ entry of the backflow rank table (Table~\ref{tab:bf_rank}), whose per-run
artefacts were not kept. The former is omitted from the quantitative interpretation because
its phase convention cannot be verified. One inconsistency also remains: the single-particle radial
diagnostics of Table~\ref{tab:radial-compact} and Appendix~\ref{app:structural-tables} were
computed in an earlier analysis pass, on runs that at the weakest traps are not the same
trained states as those behind the shell-resolved diagnostics and the radial-density
figures.
