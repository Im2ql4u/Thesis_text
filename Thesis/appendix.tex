...
among them optimistic by several hundredths of a percent. For $N{=}2$ the route
...
radial and angular melting anticipated in \S\ref{sec:wigner-markers-theory}.
...
$1200$ persistent walkers, twenty Metropolis sweeps between the two retained snapshots of each
round, about $3.6\times10^3$ rows per step, the exact Laplacian. Its phases are timed inside the
...
To separate the paradigm from the optimiser, Table~\ref{tab:walltime-grid} runs all four
combinations on one network (CTNN V-cycle correlator and CTNN backflow, $108$k parameters) with
...
\paragraph{Code.} All code used in this thesis---the ansatz, both training routes, the
structural and tangent-space diagnostics, the evaluation scripts and the figure scripts---is
public at \url{https://github.com/Im2ql4u/Thesis}, together with the run configurations,
the metadata of every saved model and the derived tables from which the numbers in
Chapters~\ref{ch:results} and~\ref{ch:kernel} are quoted. The text alone, with the figures it
includes, is at \url{https://github.com/Im2ql4u/Thesis_text}.
...
artefacts were not kept. One inconsistency also remains: the single-particle radial
