...
training at all, and carried a Markov-chain-free cascade of converged states from
$\omega\approx0.01$ to $\omega=0.0035$.
...
expressivity. We have not shown the ansatz cannot represent the $\omega=0.0027$ state, and
...
energy (Table~\ref{tab:reliability}). The wall at $0.0035$ belongs to the cascade, and the
...
That radial and angular order need not set in together is not new either: path-integral
simulations of mesoscopic two-dimensional clusters already separate orientational from radial
melting~\cite{Filinov_2001}, with the orientational order there taken between shells. What the
topology-resolved diagnostics here add is the same separation \emph{within} the shells of the
ground state, topology by topology---radial expansion and shell stiffening completing well
before $n$-fold order appears on each ring---because they condition on the shell partition
before measuring angular correlation instead of averaging over it. The $N{=}6$ ring approaches its classical radius
