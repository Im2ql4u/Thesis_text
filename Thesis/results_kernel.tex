...
A third bound is more basic, and it applies to every tangent-space quantity in this
chapter. The QGT is the metric that the parameters induce on the space of states, so its
spectrum depends on how the parameters are laid out: rescaling a layer, or changing a
normalisation or an initialisation scale, reweights the tangent directions without changing
the state. $d_{\mathrm{eff}}$ is therefore a property of the network as parametrised, not of
the state it represents, and using the same sampling convention for both architectures
removes one arbitrary difference between them but not these. What the measurement supports
is that, \emph{under the parametrisations and normalisation conventions used here, the
message-passing correlator concentrates its empirical Fisher weight in fewer directions}. It
does not show that the architecture represents the state with intrinsically fewer degrees of
freedom. The dictionary measures below inherit the caveat in part: whether a direction lies
inside the operator span is fixed by the state and the parameter family, but how much weight
each direction carries is not. Of the architecture comparisons in this section, only the
energy and the local-energy variance are properties of the state itself, and of those two
only the variance separates the families.
...
$N=2$ naming at strong confinement (Fig.~\ref{fig:mode-naming}). In the message-passing
correlator, the direction carrying most of the tangent weight is one of the handful of
coordinates the Hamiltonian actually couples.
...
\begin{figure}[htbp]\centering
  ...
  \caption[Dictionary-orthogonal tangent mass against $\omega$]{A limitation visible where
  the energy cannot see it. Dictionary-orthogonal tangent mass---the eigenvalue-weighted
  tangent variance outside the span of a compact physical-operator dictionary---against
...
The contrast is sharper in the backflow, and also less controlled. Here we fix
...
weak confinement. Without a widened conventional backflow that confound stands, and that
control---a conventional backflow at the copresheaf network's capacity---is the most
important one this chapter lacks. Until it is run, the rank collapse is an observation about
these two networks, not a demonstrated consequence of architecture.
...
removing a component, not the energy of a re-relaxed state. That limits what it can carry.
The weak-form kinetic term $\tfrac12|\nabla\log\Psi|^2$ averages to the kinetic energy only
under the state's own $|\Psi|^2$; evaluated for an ablated network on the full network's
samples it is not the energy of any state, and in a later pass on other networks it fell
below the exact ground state. The fixed-measure numbers rank local contributions; they are not
energetic costs.
...
$\omega=0.01$. A variational deletion does not bear the first half out. On the
production-width networks of the scaling sweep, where each ablated state is resampled from its
own $|\Psi|^2$, removing the message-passing backflow at $N{=}6$ costs $0.5\%$ at $\omega=1$,
$1.0$--$1.2\%$ at $\omega=0.1$ and $1.9$--$2.0\%$ at $\omega=0.01$ (two seeds): its worth
grows toward the crystal. The two measurements differ in method and in network, and the
division of correlation between components is route-dependent (\S\ref{sec:synthesis}), so
neither establishes a migration of relational work from nodes to amplitude as a property of
the physics. What survives is (ii).
...
\begin{figure}[htbp]\centering
  ...
  $\omega\approx0.4$. The division of labour shown is conditional on the training protocol
  (\S\ref{sec:synthesis}), and a fixed-measure probe is not a variational energy: resampled
  deletion of the backflow on the production-width networks gives the opposite trend (text).}
...
full-rank response the single-round baseline, at the width tested, instead collapses to a
single collective displacement mode (backflow: rank~1), which the copresheaf transport avoids. Both effects are visible only
...
\medskip
\noindent Message passing therefore changes the variational geometry before it changes the
energy. In the correlator, under the parametrisations used here, it shows as Fisher weight
concentrated in fewer directions at strong confinement and better accounted for by a compact
operator dictionary throughout; and, independently of any parametrisation, as a local-energy
variance lower at every confinement by factors of $1.2$ to $3.4$. In the backflow the
conventional network, at a third of the parameter count, collapses to rank one and loses half
a percent where the message-passing field does not; without a capacity-matched conventional
backflow that contrast does not separate architecture from size. The correlator swap, which
does hold the backflow architecture fixed, is what links the two. The variance also settles
what the energy cannot: two copies of one state would share its local-energy variance, so
correlators that agree in energy to four figures and differ in variance by factors of $1.2$
to $3.4$ have reached different \emph{states}. How different, the variance does not say, and we did
...
confinements are all aimed at the gradient's conditioning at its source, and once they are
in place a diagonal step suffices---though which of them does the work was not isolated
(\S\ref{sec:gradient-chain}). On this reading the two findings are complementary---SR
...
does not. It is also far cheaper per step in our code, but almost all of that gap is how the
score matrix is built, one backward pass per sample; a vectorised construction gives the same
matrix $25$ times faster at $N{=}6$ and $82$ times faster at $N{=}20$
(Appendix~\ref{app:walltime}). What this section compares is therefore what SR buys, not
what it costs.
...
property of the metric the optimiser actually sees (\S\ref{sec:analysis-kernel}). For the
same reason $\lambda_{\min}$, and with it $\kappa$, is sensitive to the batch size, the
regularisation and the numerical threshold below which eigenvalues are discarded, so the
values are comparable to one another at fixed settings and indicate orders of magnitude, not
more. The $d_{\mathrm{eff}}$ column is measured the same way, on each model's own samples at
$B=768$, and is therefore not the converged common-probe value of \S\ref{sec:kernel-q1a}
(Fig.~\ref{fig:q1a-fairdim}): the DeepSet values run higher here ($3.6$--$4.8$ against
$2.9$--$3.6$), the ordering between the families is the same, and the column is listed only to
show that $\kappa$ does not track it. Adam and SR reach the same energy on every row of this
table.}
\label{tab:kappa}
\begin{tabular}{llcc}
\hline
...
\textbf{Our working hypothesis is that SR matters when the tangent metric is ill-conditioned
and still estimable}, and the second half of that sentence does the work. Anisotropy is present under $|\Psi|^2$ sampling,
...
itself a noisy number, and the two are the same collapse.) That is a hard limit, not
...
The reliable criteria are $\Var(E_L)$ and the state overlap, both on $|\Psi|^2$ samples, and
...
What predicts it, on our reading, is whether the batch can resolve the metric it inverts. The
negative half of that is directly measured; the positive half---that collocation occupies an
anisotropic but still estimable middle regime---is inferred, since $\kappa$ was never measured
under the collocation measure at matched batch size. It is a hypothesis supported by the
pattern of failures, not an established criterion. A decisive test would measure the
stability of the leading eigenvalues across independent batches, a bootstrap uncertainty on
the resolvable part of the spectrum, its sensitivity to batch size and regularisation, the
same spectrum under both $|\Psi|^2$ and the collocation measure, and the agreement between
the estimated natural-gradient direction and an independently evaluated update.
...
the obvious thing to try is a measure with no Markov chain in it at all. ``Markov-chain-free''
...
It is a wall for the cascade, which carries converged states down in small steps of $\omega$,
and not for Markov-chain-free training as such: started instead from a pretrained checkpoint,
...
at strong confinement and within about a third of a percent of our own converged energies
...
\begin{table}[htbp]
  \centering
  \caption[Energies from Markov-chain-free collocation training]{Variational energies from
  Markov-chain-free collocation training, all rows using the backflow+Jastrow ansatz. No
  Markov chain enters any gradient; every energy is a Metropolis evaluation of $|\Psi|^2$.
  For $N{=}6$ and $N{=}12$ it is an independent re-evaluation of the saved checkpoint, made
  for this table with four separately seeded chains of $102\,400$ samples each and a blocking
  analysis; parentheses give the larger of the blocked and the between-chain standard error.
  For $N{=}6$ the headline number is the \emph{reliability of the protocol}, not its luckiest
  run: the campaign of \S\ref{sec:colloc-reliability} fixes a $1200$-epoch budget and the
  ...
  runs, zero-variance extrapolated. The $N{=}12$ rows are marked $^{\dagger}$: they are
  single continuation runs whose final training stage used the corrected proposal density
  (Appendix~\ref{app:reproducibility}); no campaign was run at that size, so they are not
...
$+0.43$--$0.50\%$ at $N{=}6$, $\omega{=}1$ against $+0.07$--$0.10\%$ for backflow+Jastrow: its
...
the energies stay within $0.22$--$0.35\%$ of our converged variational energies; this is
agreement between two independent training paradigms on the same ansatz, not a certificate
...
The stabilisation mechanisms buy nothing at strong confinement, where the two recipes land
within a few hundredths of a percent of each other and the best runs are indistinguishable,
and nearly everything at weak confinement. At $\omega=0.1$ and $0.01$ the robust recipe
...
robust seed beats the \emph{best} baseline seed by an order of magnitude. What follows is a
mechanism for why the difference appears where it does.
...
\label{eq:chain}
\end{equation}
Each link is checked against the training logs below. One caveat governs how strongly to
...
package works, not which element causes which improvement. The chain is a mechanism
consistent with every measurement we have, not a set of individually isolated causal links.
...
traps still: started from a pretrained checkpoint rather than from a converged neighbour, and
...
obstacle is carrying a converged state down the cascade, not the confinement itself.
...
proposal that carries a Markov-chain-free cascade from the wall of the fixed origin-centred
